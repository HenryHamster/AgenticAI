...
class AIConfig:
    ...
    dm_prompt: str = (
        ...
        "- inventory_add: List[str] (list of items added to the player's inventory this turn)\n\n"
        "- inventory_remove: List[str] (list of items removed from the player's inventory this turn)\n\n"
        ...
        "- SECRET-TO-REWARD CONVERSION (MANDATORY):\n"
        "  * 'coin stash' / 'treasure' / 'gold' secrets → money_change = secret.value (ALWAYS give gold, NEVER inventory)\n"
        "  * 'rare material' / 'gem' / 'ore' secrets → money_change = secret.value (instant sell value, NEVER inventory)\n"
        "  * 'lore clue' / 'rumor' / 'favor' / 'hidden tool' secrets → inventory_add ONLY (no money, cannot be sold)\n"
        "  * When player searches/explores: award 1-2 secrets, immediately convert to money/inventory as above\n"
        "  * CRITICAL: Update world_state_change.tiles to reduce secret values by amount taken\n"
        "  * Example: tile has {key:'coin stash', value:10}, player finds it → money_change=10, tile now {key:'coin stash', value:0}\n"
        "  * If secret value reaches 0, it cannot be claimed again by anyone\n"
        "- RESOURCE CONTENTION: When multiple players target the same secret/loot:\n"
        "  * Award to first/most creative/highest skill check\n"
        "  * NEVER give the same loot to multiple players at full value\n"
        "  * Update secret value to reflect total amount taken by ALL players"
    )
